...
// Tu token secreto para el endpoint admin (cámbialo por uno seguro y guárdalo en una variable de entorno)
// const ADMIN_TOKEN = 'a1b2c3d4e5f6789012345678901234567890abcd'; // Opción hardcoded (menos segura)
const ADMIN_TOKEN = process.env.ADMIN_API_KEY; // Recomendado: usar variable de entorno
...
const StreamSchema = new mongoose.Schema({
  ...
  cedulaUsuario: { type: String, required: true, ref: 'User' }, // Referencia al usuario
...
// Para producción, usar una base de datos como Redis sería más robusto
...
app.post('/admin/register-user', async (req, res) => {
  ...
  try {
    ...
    const response = await axios.get(`https://api.cedula.com.ve/api/v1`, { // Corrección: Eliminar espacio
      params: {
    ...
    console.log('📄 Respuesta de API de cédula:', JSON.stringify(response.data, null, 2));

    // Manejo de errores de la API de cédula
    if (response.data.error) {
      console.error('❌ Error de la API de cédula:', response.data.error);
    ...
    if (!response.data.data || !response.data.data.primer_nombre || !response.data.data.primer_apellido) {
        console.error('❌ Estructura de respuesta inesperada de la API de cédula.');
    ...
    // Notificar por WhatsApp (opcional, puedes comentar esta línea si no quieres el mensaje)
    ...
    if (error.code === 11000) { // Error de clave duplicada de MongoDB
      console.warn(`⚠️ Intento de registro duplicado para la cédula ${cedula}.`);
...
// --- INICIO DEL SERVIDOR ---
app.listen(PORT, () => {
  console.log(`🚀 Servidor AQUITA WhatsApp Bot corriendo en http://localhost:${PORT}`);
});
